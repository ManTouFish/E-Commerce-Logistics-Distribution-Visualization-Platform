...
const OrderMap: React.FC<ExtendedOrderMapProps> = ({
  ...
  // ---------------------------------------------------------------------------
  useEffect(() => {
    if (!isMapReady || !mapRef.current || !AMapRef.current || !trajectories?.length)
      return;

    const AMap = AMapRef.current;
    const map = mapRef.current;

    const path: LngLatTuple[] = [...trajectories]
      .reverse()
    ...
    if (!carMarkerRef.current) {
      ...
      if (followCar && !userInteractedRef.current) {
        ...
        map.setFitView(
          [polylineRef.current, carMarkerRef.current],
          false,
          [50, 50, 50, 50]
        );
      }
    ...
    const isNewPoint =
      prevLatest &&
      (prevLatest.longitude !== latestPoint[0] ||
        prevLatest.latitude !== latestPoint[1]);
    ...
    if (isNewPoint && prevLatest) {
      ...
      const animate = (timestamp: number) => {
        ...
        const currentLng =
          startPos.longitude + (endPos.longitude - startPos.longitude) * ease;
        const currentLat =
          startPos.latitude + (endPos.latitude - startPos.latitude) * ease;
